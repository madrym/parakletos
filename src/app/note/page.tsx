...
const highlightColors = [
  'bg-yellow-200', 'bg-green-200', 'bg-blue-200', 'bg-red-200',
...
export default function NotePage() {
  ...
  const toggleHighlight = () => {
    const anyHighlighted = sections.some(section =>
      section.verses.some(verse => selectedVerses.includes(verse.number) && verse.highlight)
    );
  ...
  const addBibleNote = () => {
    if (currentNote.trim() && selectedVerses.length > 0) {
      setBibleNotes(prev => [...prev, { verseNumbers: selectedVerses, text: currentNote }]);
      setCurrentNote('');
      setSelectedVerses([]);
  ...
  const getNotesForVerse = (verseNumber: number) => {
    return bibleNotes.filter(note => note.verseNumbers.includes(verseNumber));
  };

  const copySelectedVerses = () => {
    const textToCopy = sections
      .flatMap(section => section.verses)
      .filter(verse => selectedVerses.includes(verse.number))
  ...
  const isAnySelectedVerseHighlighted = () => {
    return sections.some(section => 
      section.verses.some(verse => 
        selectedVerses.includes(verse.number) && verse.highlight
      )
    );
  };
      ...
      <div className="min-h-screen bg-emerald-50 p-4 relative">
        <div className="max-w-full mx-auto bg-white shadow-lg rounded-lg overflow-hidden mb-4">
          <div className="p-4 bg-emerald-100 text-black">
            ...
            />
            <p className="text-xs md:text-sm text-emerald-700 mb-4 ml-1">{createdAt}</p>
            <Dialog>
                ...
                <div className="flex">
                  <div className="w-8 flex-shrink-0">
                    {section.verses.map((verse) => {
                      ...
                      return (
                        <div key={verse.number} className="h-6 flex items-center justify-center">
                          {notes.length > 0 && (
                            <Popover>
                                ...
                                {notes.map((note, noteIndex) => (
                                  <div key={noteIndex} className="mb-2 last:mb-0">
                                    <p className="font-sans text-sm">
                                      <span className="font-bold">Verses {note.verseNumbers.join(', ')}:</span> {note.text}
                                    </p>
                                    {noteIndex < notes.length - 1 && <hr className="my-2" />}
                                  </div>
                                ))}
                              </PopoverContent>
                            </Popover>
                        ...
                        <React.Fragment key={verse.number}>
                          <sup 
                            className={`text-xs mr-1 font-sans cursor-pointer ${
                              selectedVerses.includes(verse.number) ? 'bg-gray-200' : 'text-gray-500'
                            }`}
        ...
        {showToolbar && (
          ...
          >
            <Dialog open={isAddingNote} onOpenChange={setIsAddingNote}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Add Note for Verses {selectedVerses.join(', ')}</DialogTitle>
                </DialogHeader>
                <Textarea
                  value={currentNote}
                ...
                />
                <Button onClick={addBibleNote} disabled={!currentNote.trim()}>Add Note</Button>
              </DialogContent>
            </Dialog>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button 
                  ...
                  className={isAnySelectedVerseHighlighted() ? 'bg-gray-200' : ''}
                >
                  <Highlighter className="h-4 w-4" />
                </Button>
              </TooltipTrigger>
              <TooltipContent>
                {isAnySelectedVerseHighlighted() ? 'Remove Highlight' : 'Add Highlight'}
              </TooltipContent>
            </Tooltip>
            <Popover>
              <Tooltip>
        ...
        <div className="mt-4">
          ...
          {bibleNotes.map((note, index) => (
            <div key={index} className="bg-gray-100 p-2 rounded mb-2">
              <p className="font-bold">Verses: {note.verseNumbers.join(', ')}</p>
              <p>{note.text}</p>
            </div>
          ))}
        </div>
